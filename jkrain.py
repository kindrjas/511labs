...
def leap_frog(u, h, gu, gh, n_grid):
    ...
    for pt in np.arange(1, n_grid - 1):
        u.next[pt] = u.prev[pt] - gu * 2 * (h.now[pt + 1] - h.now[pt])   #modified h.now[pt-1] to h.now[pt] and mult by 2
        h.next[pt] = h.prev[pt] - gh * 2 * (u.now[pt] - u.now[pt-1])   #modified u.now[pt-1] to h.now[pt]
