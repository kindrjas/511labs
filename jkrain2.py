...
def initial_conditions(u, v, h, ho):
    ...
    added a new argument for v
    """
    u.prev[:] = 0
    v.prev[:] = 0 # added a spot for v
    h.prev[:] = 0
...
def boundary_conditions(u_array, v_array, h_array, n_grid):
    ...
    Modified to accept new argument for v_array
    """
    u_array[0] = 0
    u_array[n_grid - 1] = 0
    v_array[0] = 0 #added v_array
    v_array[n_grid - 1] = 0 #added v_array
    h_array[0] = h_array[1]
...
def first_time_step(u, v, h, g, H, dt, dx, ho, gu, gh, n_grid):
    ...
    added v as an argument
    """
    u.now[1:n_grid - 1] = 0
    factor = gu * ho / 2
    midpoint = n_grid // 2
    #u.now[midpoint - 1] = -factor   # made this line not read
    #u.now[midpoint + 1] = factor    # made this line not read
    u.now[2]= -ho * g * dt / dx      # added this
    u.now[3]= -u.now[2] * dt         # added this
    v.now[1:n_grid - 1] = 0 # initialized v to zero
    h.now[1:n_grid - 1] = 0
    h.now[5] = ho - g * H * ho * dt ** 2 / (dx ** 2)   # Removed 4 from * dx**2, made h.now[midpoint] be h.now[3]


def leap_frog(u, v, h, gu, fdt, gh, n_grid):
    """Calculate the next time step values using the leap-frog scheme
    derived from equations 4.16 and 4.17.

    added argument for v
    """
    for pt in np.arange(1, n_grid - 1):
        u.next[pt] = u.prev[pt] - (gu * 2 * (h.now[pt + 1] - h.now[pt])) + (2 * fdt * v.now[pt])  # modified for new equations
        v.next[pt] = v.prev[pt] - (2 * fdt * u.now[pt])   # added equation 2 for v
        h.next[pt] = h.prev[pt] - gh * 2 * (u.now[pt] - u.now[pt-1])   # no change needed
...
def make_graph(u, v, h, dt, n_time):
    ...
    fig, (ax_u, ax_v, ax_h) = plt.subplots(3,1, figsize=(10,10)) # added slot for v
    ...
    for time in range(0, n_time, interval):
        ...
        ax_v.plot(v.store[:, time], color=colorVal) # added spot for v
...
#     n_time, n_grid = map(int, args)
    ...
    f = 0.0001098               # coriolis force at 49 parallel [s^-1] (added)
    fdt = f * dt                # third handy constant added
    # Create velocity and surface height objects
    u = Quantity(n_grid, n_time)
    v = Quantity(n_grid, n_time)   # added slot for v
    h = Quantity(n_grid, n_time)
    # Set up initial conditions and store them in the time step
    # results arrays
    initial_conditions(u, v, h, ho) #added slot for v
    u.store_timestep(0, 'prev')
    v.store_timestep(0, 'prev')   #added line for v
    ...
    first_time_step(u, v, h, g, H, dt, dx, ho, gu, gh, n_grid)  # added v
    boundary_conditions(u.now, v.now, h.now, n_grid)   #added spot for v
    u.store_timestep(1, 'now')
    v.store_timestep(1, 'now')  # added line for v
    ...
    for t in np.arange(2, n_time):
        ...
        leap_frog(u, v, h, gu, fdt, gh, n_grid) # added v and fdt
        boundary_conditions(u.next, v.next, h.next, n_grid)  #added v
        # Store the values in the time step results arrays, and shift
        # .now to .prev, and .next to .now in preparation for the next
        # time step
        u.store_timestep(t)
        v.store_timestep(t)  # added v
        h.store_timestep(t)
        u.shift()
        v.shift()   # added v
        h.shift()
    ...
    make_graph(u, v, h, dt, n_time)   #added slot for v
    return
