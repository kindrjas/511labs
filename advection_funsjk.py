...
# create a matrix to store the table to be read in
    ...
    #os.getcwd()
    fname = '/Users/jasonkindrachuk/eosc511/numeric/numlabs/lab10/Tables/l{0}_table.txt'.format(order)
    fp = open(fname, 'r')
    print(fname)
...
def make_graph(cmatrix, timesteps, Numpoints, dt, order):
    ...
    the_title = fig.text(0.25, 0.95, 'Order %.0f: Concentrations Results from t = %.3fs to %.3fs' % (order, 0, dt*timesteps))
...
def advection3(timesteps, order):
    ...
    make_graph(cmatrix, timesteps, Numpoints, dt, order)
